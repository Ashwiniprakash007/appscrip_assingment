            ...
            <main id='filter-bar-top-section'>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', cursor: 'pointer', color: '#888792' }} onClick={() => {
                        ...
                        <Image src={leftArrow} alt='arrow-left-icon' />
                ...
                <section className='recomended-section' style={{ position: 'relative', cursor: 'pointer', paddingRight: '12px' }}>
                    <div className='recomended-head'>
                        <p>RECOMENDED</p>
                        <Image src={downArrowLogo} alt='arrow-down-icon' />
                    </div>

                    <div className='recomeded-filter-dropdown'>
                        <div style={{ display: 'flex', gap: '5px', alignItems: 'center', justifyContent: 'flex-end' }}>
                            <Image src={tickMarkLogo} alt='tick-mark-icon' style={{ height: '26px', width: '26px' }} />
                ...
                <>
                    <div className='filter-item' key={id}>
                        <div className='filter-title'>
                            <p>{title}</p>
                            <Image src={downArrow} alt='Dropdown Menu Arrow' onClick={() => showCategoriesHandler(id)} />
                        </div>
                        ...
                        {dropdownCategories[id] &&
                            <form className='filter-show-form' key={`form-${id}`}>
                                <div className='filter-show-title'>
                                    <p>Unselect all</p>
                                </div>
                                <div className='filter-checkboxes' key={`checkbox-men-${id}`}>
                                    <input type='checkbox' id={`men-${id}`} /> <label htmlFor={`men-${id}`}>Men</label>
                                </div>
                                <div className='filter-checkboxes' key={`checkbox-women-${id}`}>
                                    <input type='checkbox' id={`women-${id}`} /> <label htmlFor={`women-${id}`}>Women</label>
                                </div>
                                <div className='filter-checkboxes' key={`checkbox-boys-kids-${id}`}>
                                    <input type='checkbox' id={`boys-kids-${id}`} /> <label htmlFor={`boys-kids-${id}`}>Boys & Kids</label>
                                </div>
                            </form>
                        }
                    </div>
                                    ...
                                    <div className='product-container'>
                                        <div>
                                            ...
                                            <div className='account-creation-info'>
                                                ...
                                                <Image src={heartLogo} alt='like-icon' />
                                            </div>
                                        </div>
                                    </div>
                                </section>
